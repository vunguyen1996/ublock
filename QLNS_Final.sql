...
WHILE EXISTS(select NULL from sys.databases where name='QLNS_Final')
BEGIN
    DECLARE @SQL varchar(max)
    SELECT @SQL = COALESCE(@SQL,'') + 'Kill ' + Convert(varchar, SPId) + ';'
    FROM MASTER..SysProcesses
    WHERE DBId = DB_ID(N'QLNS_Final') AND SPId <> @@SPId
    EXEC(@SQL)
    DROP DATABASE [QLNS_Final]
END
GO

/* Collation = SQL_Latin1_General_CP1_CI_AS */
CREATE DATABASE [QLNS_Final]
GO
